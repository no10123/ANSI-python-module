...
DOC_STYLE = {
    ...
    # asked for / written back by the .F form — subset of section_order
    "form_sections": ["Args", "Returns", "Example", "Notes"],

    # canonical name -> acceptable header text (first alias = what .F writes back)
    "aliases": {
        "Args":    ["Args", "Arguments", "Parameters", "Params"],
        "Returns": ["Returns", "Return"],
        "Yields":  ["Yields", "Yield"],
        "Raises":  ["Raises", "Except", "Exceptions"],
        "Example": ["Example", "Examples"],
        "Notes":   ["Notes", "Note"],
    },
    # how each section's body is structured
    "kind": {
        "Args": "args", "Returns": "text", "Yields": "text",
        "Raises": "text", "Example": "example", "Notes": "text",
    },
    # colors keyed by the same "cX" strings pretty_doc already resolves
    "colors": {
        "Args": ["cn", "ct"], "Returns": ["ct"], "Yields": ["ct"],
        "Raises": ["ct"], "Example": ["cc"], "Notes": ["ct"],
    },
    "desc_color": ["ct"],
    "typed_section": "Args",                                    # which section gets [type] auto-inserted

    "header_pattern": r"^[A-Za-z][A-Za-z ]*:\s*$",              # what a header line looks like
    "underline_pattern": None,                                  # e.g. r"^-{3,}\s*$" for numpy style
    "example_prefixes": (">>> ", "... "),                       # what to prefix example with
    "indent_size": 4,                                           # how many spaces in a tab
}
...
def style_doc_helper(style:DocStyle) -> tuple[dict[str, str], list[str], dict[str, list[str]]]:
    ...
    for canonical in style["section_order"]:
        colors = style["colors"].get(canonical, ["ct"])
        for alias in style["aliases"][canonical]:
            key = f"{alias}:"
            alias_lookup[key.lower()] = canonical
            headers.append(key)
            color_d[key] = colors
    return alias_lookup, headers, color_d
...
def _write_docstring_to_source(func: Callable[..., Any],body_lines: list[str],) -> tuple[bool, str]:
    ...
    Returns:
        bool, str = success?, error msg | filepath

    Example:
        >>> _write_docstring_to_source(globals()[input("func: ")], input("docstring: "))
    ...
    if not filepath:
        return False, "could not locate source file (built-in or dynamically defined)."

    try:
        tree = ast.parse("".join(src_lines))
    except (SyntaxError, IndentationError) as e:
        return False, f"could not parse function source (nested/indented func?): {e}"

    func_node = next((n for n in ast.walk(tree) if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == func.__name__),None)
    if func_node is None or not func_node.body:
        return False, "could not locate function body."
    first_stmt = func_node.body[0]
    body_indent = " " * first_stmt.col_offset

    has_docstring = (isinstance(first_stmt, ast.Expr) and isinstance(getattr(first_stmt, "value", None), ast.Constant) and isinstance(first_stmt.value.value, str))

    new_block = [f'{body_indent}"""\n']
    new_block += [f"{body_indent}{line}\n" if line else "\n" for line in body_lines]
    new_block.append(f'{body_indent}"""\n')

    offset = start_lineno - 1

    try:
        with open(filepath, "r", encoding="utf-8") as f:
            file_lines = f.readlines()
    except UnicodeDecodeError as e:
        return False, f"file isn't valid UTF-8 ({e}); refusing to risk corrupting it."

    if has_docstring:
        doc_start = first_stmt.lineno + offset
        doc_end = first_stmt.end_lineno + offset
        new_file_lines = file_lines[:doc_start - 1] + new_block + file_lines[doc_end:]
    else:
        insert_at = first_stmt.lineno + offset - 1
        new_file_lines = file_lines[:insert_at] + new_block + file_lines[insert_at:]
...
def doc_cmd(
    ...
    while True:
        ...
        if current_view in [".menu",".m","\n"]:
            ...
            current_view = ".back"
            continue
        elif current_view.startswith((".F", ".form")):
            cvl = current_view.split(" ")
            if len(cvl) > 1:
            ...
            if not callable(func_obj):
                ...
                try:
                    ...
                    params = {
                        p: (param.annotation if param.annotation != inspect.Parameter.empty else None)
                        for p, param in sig.parameters.items()
                    }
                except (ValueError, TypeError):
                    params = {}

                HL, TY, R = rgb(100, 255, 150), rgb(150, 150, 255), "\033[0m"
                ...
                def ask(prompt, multiline=False):
                    print(f"{HL}{prompt}{R}")
                    if not multiline:
                        return input(f"{cn}>  {R}").strip()
                    print(f"{rgb(120, 120, 120)}  (multi-line, empty line to finish){R}")
                    lines = []
                    while True:
                        line = input(f"{cn}>  {R}")
                        if line == "":
                ...
                def ask_or_keep(prompt, existing_lines):
                    if existing_lines and any(l.strip() for l in existing_lines):
                        print(f"{HL}{prompt}{R}")
                        preview = " ".join(l.strip() for l in existing_lines if l.strip())
                        preview = (preview[:77] + "...") if len(preview) > 80 else preview
                        print(f"{rgb(120, 120, 120)}  existing: \"{preview}\"{R}")
                        choice = input(f"{cn}  [K]eep / [O]verwrite? {R}").strip().lower()
                        if choice in ("", "k", "keep"):
                ...
                if save in ("y", "yes"):
                    ...
                    print(f"{rgb(150, 150, 150)}[*] kept in-memory only (this session).\033[0m\n")
...
def floop(func, args = (), kwargs = None, raw=True):
    ...
    Args:
        func:
            the func you want to loop
        args:
            any args you want to call the function with.
        kwargs:
            any kwargs you want to call the function with.
        raw:
            if you want the looping to use raw terminal
